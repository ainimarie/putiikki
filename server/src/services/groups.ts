...
export async function addGroup(group: GroupRequest) {
  ...
  try {
    ...
    await update(
      `INSERT INTO group_members (group_id, customer_id, is_leader) VALUES ($1, $2, $3)`,
      [updateResult.rows[0].id, user.id, true]
    );
  } catch (error) {
    console.log(error);
...
export async function getGroupIdFromUuid(uuid: string): Promise<number> {
  ...
  console.log("moi2, getGroupIdFromUuid");
...
export async function getGroupId(name: string): Promise<number> {
  const query = await getOne("SELECT id FROM groups WHERE name = LOWER($1)", [
    name,
  ]);
  if (query === undefined) {
    throw new Error("Group not found");
...
export async function getGroupWithMembers(groupId: string) {
  console.log("getGroupWithMembers", groupId);
  const query = await getMany(
    "SELECT g.id, g.name, g.description, g.uuid, c.id AS member_id, c.username, c.name AS member_name, gm.points, gm.is_leader FROM groups g JOIN group_members gm ON g.id = gm.group_id JOIN customers c ON gm.customer_id = c.id WHERE g.uuid = $1",
    [groupId]
  );
...
export async function getMemberWithinGroup(uuid: string, username: string) {
  ...
  const query = await getOne(
    "SELECT c.id AS member_id, c.username, c.name AS member_name, gm.points, gm.is_leader FROM groups g JOIN group_members gm ON g.id = gm.group_id JOIN customers c ON gm.customer_id = c.id WHERE g.id = $1 and c.username = $2",
    [groupId, username]
  );
...
export async function updateUserPoints(
  ...
  const query = await update(
    "UPDATE group_members SET points = points + $1 WHERE group_id = $2 AND customer_id = (SELECT id FROM customers WHERE username = $3)",
    [points, groupId, username]
  );
  if (query.rowCount === 0) {
